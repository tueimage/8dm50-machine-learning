...
\end{equation**}
where $b_j$ are the unknown parameters or coefficients. 
...
With \textit{logisitic regression} this probability is modeled as a \textit{logistic function}:
...
Using K-1 log-odds satisfies the constrain that probabilities sum to one. The choice of the last class to be used at denominator is arbitraty and does not affect the result.
...
It is easy to see that probabilites sum to one by rewriting the model as:
...
An alternative notation for the probabilities is $P(G=k|X=x) = p_k(x;\theta)$, to emphasize the dependence on the entire parameter set $\theta=\{ \beta_{10}, \beta_1_T, \dots, \beta_{(K-1)0, \beta_{K-1}^T}\}$.
...
\begin{equation*}
    l(\theta) = \sum_{i=1}^N \log p_g_i(x_i; \theta)
\end{equation*}
