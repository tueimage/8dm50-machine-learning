...
Ask question on the 2d space divided not by decision trees
Show example with diagonally separable data and 2 example of decision boundary (diagonal and tree)
\end{frame}
...
\item Each leaf has one observation only (T)
...
\item It is not possible to tell without seeing the data (T)
\end{itemize}	
\end{frame}
...
\vspace{0.5cm}
ToDo: show example with classification (show one region and say what is the corresponding class)
\end{frame}

\begin{frame}{Question }
ToDo: question about overfitting (What is the risk of a tree with a lot of leaves)
\end{frame}
