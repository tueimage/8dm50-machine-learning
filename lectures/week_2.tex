...
\frametitle{Weight decay}
    \begin{itemize}
        \item Consider the linear regression example
        \item We can modify the minimal squared error criterion by adding an additional condition
        \item 
        We define a new function $J(\vect{w}$ which can express preference for the weights to have smaller $L^2$ norm:
        $$
            J(\vect{w}) = MSE){\mbox{train}} + \lanbda \vect{w}^T\vect{w}
        $$
        where $\labda$ is a previously fixed parameter which expresses the strength of our preference for smaller weights
        \item $\lambda = 0$ means no preference, the larger $\lambda$ the smaller the weights 
        \item The choice is between fitting the training data and being small
    \end{itemize}
\end{frame}
...
\frametitle{Regularization}
    \begin{itemize}
        \item The waight decay is an example of a {\bf regularization}
        \item Regularization is any modification of the learning algorithm in order to minimze the generalization error but not the training error
        \item More generally, we can regularize a model that learns a function $f(\vect{x};\vect{\theta})$ by adding to the {\bf cost function} which we are minimizing a penalty called a {\bf regularizer}
        \tem In the case of linear regression and weight decay, we added to $\mbox{MSE}$ cost function the regularizer $\Omega(\vect{w}) = \vect{w}^T\vect{w}$
        \item In this course we will see other examples of regularization
    \end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Regularization for deep learning}
    \begin{itemize}
        \item Central problem in machine learning: How to design our algorithms/model such that it will perform good not only on the training data, but also on new inputs, i.e., how to ultimately minimize the test error
        \item Many strategies reduce the test error on the expense of the training error
        \item A collective name for this strategies is {\bf regularization}
        \item Definition: \\
        Any modiﬁcation of the learning algorithm intended to reduce its generalization (test) error but not (necessarily) its training error
    \end{itemize}
    
\end{frame}

\section{Bootstrapping, crossvalidation}
%covered in Model selection DL book 5.3 


%\begin{frame}
%\frametitle{Example: linear regression as maximum likelihood}
%    \begin{itemize}
%        \item linear regression : an algorithm that learns to take an input $\vect{x}$ and produce output %$\hat{y}$
%        \item This function from $\vect{x}$ to $\hat{y}$ is chosen to minimize the mean squared error
%        \item This criterion was introduced more or less arbitrarily
%        \item We revisit linear regression from the point of view of maximal likelihood
%    \end{itemize}
...
\begin{frame}
\frametitle{Classification outcomes}
    \begin{itemize}
    \item In practice classes $\vect{p}$ and $\vect{Y}$ have the same name and are only distinguished by the prediction context "correct" and "incorrect". The same holds for $\vect{n}$ and $\vect{N}$ 
    \item For example: In the context of a medical study the classification outcomes can be interpreted as:
    \begin{itemize}
    \item True positives: Sick people correctly diagnosed as sick \\
    \item False positives: Healthy people incorrectly diagnosed as sick \\
    \item True negatives: Healthy people correctly diagnosed as healthy \\
    \item False negatives: Sick people incorrectly diagnosed as healthy 
    \end{itemize}
    \end{itemize}
\end{frame}
...
% \frametitle{Binary classifications metrics}   
 %   \begin{itemize}
 %       \item Using the four basic categories of prediction outcomes (TP, FP, TN, FN) we can derive %various {\bf statistical measures} of performance of the classification models
 %       \item For instance the accuracy can be defined as 
 %       $$\mbox{Accuracy} = \frac{TP+TN}{TP+FP+TN+FN}$$
 %       \item Also quite fequently used measures are
 %       $$Specificity = \frac{TP}{TP+FN}  \hspace{1cm}  Sensitivity = \frac{TN}{TN+FP}$$ \\
...
\frametitle{Model evaluation}
    \begin{itemize}
        \item For tasks which do not have a discrete (categorical) output, e.g., density estimation, does not make sense to use accuracy, error rate, etc. 
        \item We need a measure which takes into account a continuous-valued score for each example.
        \item The most common measure is the {\bf average log-probability} that the  model assigns to some examples
        \item Choosing a performance measure is not always a straightforward task
        \item It could be difficult to decide what to measure and how \\
        Example of a dilemma: In regression, should we penalize the algorithm if it frequently makes medium-sized mistakes or if it rarely makes very large mistakes?
    \end{itemize}
\end{frame}
